...
--           './Data/...', which is resolved against the client's working
--           directory — running from inside /SQL will not find the file.
--
-- Usage:
...
--   - LOCAL INFILE enabled on both ends:
--       client -> --local-infile=1
--       server -> local_infile=ON
--
-- Notes:
